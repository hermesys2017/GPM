...
#         self.txt_url.setText("ftp://jsimpson.pps.eosdis.nasa.gov/data/imerg/late/")
...
#                     arg = "gdal_translate.exe HDF5:"+ "\"" + str(datasouce) +"\""+'://{0}/{1} -of GTiff {2}'.format(self.txt_hdf5_inPath.text(),self.txt_hdf_inName.text(),output)
                    arg ='gdal_translate.exe HDF5:"{0}"://{1}/{2}  -a_nodata -9999 -of GTiff "{3}"'.format(str(datasouce),self.txt_hdf5_inPath.text(),self.txt_hdf_inName.text(),output)
    ...
    def select_cmorph(self):
        try:
            ...
            for file in FileList:
                self.tb_hdf5.insertRow(count)
                self.tb_hdf5.setItem(count, 0, QTableWidgetItem(str(file)))
                self.cmorph_list.append(str(file))
...
#                         QgsMessageLog.logMessage(arg,"CMORPH step2")
#                     QgsMessageLog.logMessage(str(idx)+".bz2_{0}.asc".format(file_i),"CMORPH step2 asc")
#                     QgsMessageLog.logMessage(output,"CMORPH step2 output")
                    sub.call(arg,shell=True)
...
#                             arg = 'gdalwarp.exe -s_srs EPSG:4326 -t_srs EPSG:4326 -q -cutline "{0}" -crop_to_cutline -tr {1} {2} -dstalpha -of GTiff "{3}" "{4}"'.format(str(self.txt_Shape_path.text()),str(clipWidth),str(clipHeight),tif,Output)
...
#             QgsMessageLog.logMessage(str(check_Folder),"GPM UTM")
...
#                 arg = 'gdalwarp -overwrite -t_srs "{0}" -dstalpha -of GTiff "{1}" "{2}"'.format(str(t_srs),file,Output)
#                 if ((self.Projection).upper() != " EPSG:4326"):
#                 if (str(self.Projection).lower() == "" ):
#                     fail_file_list.append(file)
#                     _util.MessageboxShowInfo("GPM","Failed to perform normally.\n Assign projection.")
#                     self.utm_progressBar.setValue(0)
#                     return
#                 else:
#                 arg = 'gdalwarp -overwrite -s_srs "{0}" -t_srs "{1}" -of GTiff "{2}" "{3}"'.format(str(self.Projection),str(t_srs),file,Output)
                arg = 'gdalwarp -overwrite -t_srs "{0}" -of GTiff "{1}" "{2}"'.format(str(t_srs),file,Output)
#                 QgsMessageLog.logMessage(str(self.Projection),"UTM")
                exe=_util.Execute(arg)
                if (exe == 0):
                    sleep(1)
...
#             _util.MessageboxShowInfo("GPM",str(fail_file_list))
#                 arg =[
#                     osgeo4w, "gdalwarp.exe",
#                     "-overwrite","-t_srs",t_srs,
#                     "-of","GTiff",file,Output       
#                     ]
#                 QgsMessageLog.logMessage(str(arg),"Convert UTM")
#                 sub.call(arg,shell=True)
#                 sleep(1)
#                 utm_count=utm_count+1
#                 self.utm_progressBar.setValue(utm_count)
...
#                 QgsMessageLog.logMessage(str(arg),"GPM UTM")
                sub.call(arg,shell=True)
...
#             select_file = QFileDialog.getOpenFileName(self, "Select csv file.", dir, '*.csv *.CSV',options=QtGui.QFileDialog.DontUseNativeDialog)
            select_file = QFileDialog.getOpenFileName(self, "Select csv file.",'c://', '*.csv *.CSV')
            #기존 2017버전으로 복귀
...
#         dir = os.path.dirname(sys.argv[0])
        try:
            global select_SHP
            select_SHP = QFileDialog.getOpenFileName(self, "Select Shape file",'c://', '*.shp *.SHP')
...
#         global select_SHP
        ...
        try:
            self.shp_layer_polygon = QFileDialog.getOpenFileName(self, "Select Shape file", "c:\\", '*.shp *.SHP')
            # 파일을 선택했다면~
...
#         global select_SHP
        ...
        try:
            self.shp_layer_line = QFileDialog.getOpenFileName(self, "Select Shape file", "c:\\", '*.shp *.SHP')
            # 파일을 선택했다면~
...
#         global select_SHP
        ...
        try:
            self.shp_layer_point = QFileDialog.getOpenFileName(self, "Select Shape file", "c:\\", '*.shp *.SHP')
            # 파일을 선택했다면~
...
#                 symbols = baseLayer_polygon.rendererV2().symbols()
#                 sym = symbols[0]
#                 sym.setColor(QColor.fromRgb(255,0,0))
#                 baseLayer_polygon.triggerRepaint()
...
#         composerMap.setNewExtent(extent_layer)
#         composerMap.setNewExtent(QgsRectangle(xmin,ymax,ymin,xmax))
#         composerMap.zoomToExtent(QgsRectangle(xmin,ymax,ymin,xmax))
...
#         img = QImage(QSize(imageWidth_mm, imageHeight_mm), QImage.Format_RGB32)
...
#         QgsMapLayerRegistry.instance().removeMapLayers(layers)
    ...
    def Folder_List(self,type):
        try:
             folderpath  =QtGui.QFileDialog.getExistingDirectory(None, 'Select a folder:', 'c://', QtGui.QFileDialog.ShowDirsOnly)
...
#         item = QtGui.QTreeWidgetItem(self.treeWidget, ['HDF5_Convert'])
        item = QtGui.QTreeWidgetItem(self.treeWidget, ['Convert_to_TIFF'])
