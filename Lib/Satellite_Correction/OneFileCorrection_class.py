...
class satellite_correction:
    ...
    def run_correction(self, save_path, asc_file_list, groundFile_list, _decimal):
        ...
        for count in range(len(asc_file_list)):
            ...
            save_sre_correction = self.Correction_and_saveResult(
                save_path, str(groundFile_list[count]), self.sat_data, _decimal
            )
    ...
    def row_cols_calc(self, csv_file):
        ...
        csv_matrix = [
            [(self.nodata_value) for x in range(self.ncols)] for y in range(self.nrows)
        ]
        ...
        for i in range(len(split_file)):
            ...
            value_list.append(float(csv_r_value[2]))
            ...
            try:
                ...
                if csv_matrix[row_list[0]][column_list[0]] != (self.nodata_value):
                    ...
                    strInfomMsg = (
                        "Several data within the same cell. /t :: row,cols :("
                        + str(row_list[0])
                        + ", "
                        + str(column_list[0])
                        + ")"
            ...
            for xcols in range(self.ncols):
                if csv_matrix[yrow][xcols] != (self.nodata_value):
                    if 2 <= count_mat[yrow][xcols]:
                        csv_matrix[yrow][xcols] = (
                            csv_matrix[yrow][xcols] / count_mat[yrow][xcols]
                        )
        ...
        for yrow in range(self.nrows):
            for xcols in range(self.ncols):
                if groundFile_matrix[yrow][xcols] != (self.nodata_value):
                    if 2 <= count_mat[yrow][xcols]:
                        groundFile_matrix[yrow][xcols] = (
                            groundFile_matrix[yrow][xcols] / count_mat[yrow][xcols]
                        )
    ...
    def Getdist_if(self, ncols_x, nrows_y, x, y):
        ...
        try:
            if os.path.basename(self.sat_data).lower() == "yeonsei_testcase.asc":
                ...
                dist_km_between_cells = _util_sc.getdist_YSU_SampleData_Only(
                    x - ncols_x, y - nrows_y
                )  # 피타고라스, 위치로부터 거리로 계산
    ...
    def AVG_OBS_ZONE(self, asc_file):
        ...
        try:
            ...
            self.ground_avg_matrix = [
                [(self.nodata_value) for x in range(self.ncols)]
                for y in range(self.nrows)
            ]

            # 개수를 세어 둔 격자
            self.ground_count_matrix = [
                [(self.nodata_value) for x in range(self.ncols)]
                for y in range(self.nrows)
            ]  # 2017.8.22 원 : 신설
            ...
            for y in range(self.nrows):
                for x in range(self.ncols):
                        ...
                        for ncols_x in range(self.ncols):
                            ...
                            if nrows_y >= 0 and ncols_x >= 0:
                                try:
                                    ...
                                    if self.g_distance_km >= self.Getdist_if(
                                        ncols_x, nrows_y, x, y
                                    ):
                                        ...
                                        if self.ground_file[nrows_y][ncols_x] != (
                                            self.nodata_value
                                        ):
                                            intSum = (
                                                intSum
                                                + self.ground_file[nrows_y][ncols_x]
                                            )
                    ...
                    if ground_data_count > 0:
                        self.ground_avg_matrix[y][x] = (
                            self.ground_avg_matrix[y][x] / ground_data_count
                        )
                            ...
                            try:
                                if self.g_distance_km >= self.Getdist_if(
                                    ...
                                    if (self.ground_file)[nrows_y][ncols_x] != (
                                        ...
                                        AVG2DATA = (self.ground_avg_matrix)[y][x] - (
                                            self.ground_file
                                        )[nrows_y][ncols_x]
                        ...
                        if nrows_y >= 0 and ncols_x >= 0:
                            try:
                                if self.g_distance_km >= self.Getdist_if(
                                    ...
                                    AVG2DATA = (self.avg_sre_matrix)[y][x] - (
                                        self.body[nrows_y][ncols_x]
                                    )
    ...
    def cv_stdavg(self, groundFile, asc_file):
        ...
        try:
            ...
            # 최종이 되는 거리를 저장함
            self.g_final_dist = [
                [0 for x in range(self.ncols)] for y in range(self.nrows)
            ]
                ...
                for y in range(self.nrows):
                    for x in range(self.ncols):
                        if self.g_final_dist[y][x] == 0:
                            bALLok = False
                            if (
                                self.avg_sre_matrix[y][x] > 0
                                and self.stdev_sre_matrix[y][x]
                                / self.avg_sre_matrix[y][x]
                                >= thrs
                            ):
                                ...
                                if self.ground_count_matrix[y][x] == 0:  # 관측소 0개
                                    microF = (
                                        self.total_avg_obs / total_mean_sre
                                    )  # 전체 지상평균/전체 위성평균
                                    tauF = (
                                        self.total_std_obs / total_stdeb_sre
                                    )  # 전체 지상표준편차/ 전체 위성 표준편차
                                    SreC[y][x] = (
                                        ((self.body[y][x]) - total_mean_sre) * tauF
                                    ) + (total_mean_sre * microF)
                                    ...
                                    tauF = (
                                        self.stdev_obs_matrix[y][x]
                                        / self.stdev_sre_matrix[y][x]
                                    )
                                    SreC[y][x] = (
                                        ((self.body[y][x]) - self.avg_sre_matrix[y][x])
                                        * tauF
                                    ) + (self.avg_sre_matrix[y][x] * microF)
    ...
    def Correction_and_saveResult(self, save_path, groundFile, asc_file, _decimal):
        ...
        if sre_correction != False:
            ...
            performMsg = performMsg + "Inform : \n[{0}], {1}\n".format(
                _util_sc.GetFileName(asc_file), InformMsg
            )
            #             print (performMsg)
            return performMsg
