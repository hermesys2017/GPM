...
wget_path = '"' + os.path.dirname(os.path.dirname(os.path.abspath(__file__)))+"/wget.exe" + '"'
kst_list=[]

class GPM_download_Class():
    def __init__(self,userpass,userInfo,getFiledate_1,getFiledate_2,gpm_type,donwload_folder,timeLabel_type,progressbar):
        ...
        if (self.timeLabel_type == "KST"):
            ...
            if (getFiledate_1 == getFiledate_2):
                ...
                self.days=(str((datetime.strptime(getFiledate_2,'%Y-%m-%d').date())-(datetime.strptime(getFiledate_1,'%Y-%m-%d').date())).split(" ")[0])
        
            for day in range(int(self.days)+2):
                self.getFiledate= (str( datetime.strptime(getFiledate_1,'%Y-%m-%d').date() + timedelta(day)))
                self.main()
        ...
        if (self.gpm_type == "imerg/gis" or self.gpm_type=="imerg/gis/early"):
            ...
            for filename in tiff_list:
                if (self.timeLabel_type == "UTC"):
                    ...
                    count = count+1
                    QApplication.processEvents()
            QMessageBox.information(None, "GPM Download "," Download is complete. Please check the download folder")
    ...
    def get_file_list_kst(self,filename):
        ...
        donwload_list=[] ;
        dayfile = (filename.split(".")[4]).split("-")[0] +"-"+ (filename.split(".")[4]).split("-")[1]
    ...
    def get_file_list(self):
        if (self.gpm_type =="imerg/late" or self.gpm_type=="imerg/early"):
            server ="{0}/{1}/{2}/*{3}*".format(self.url,self.gpm_type,self.year+self.month,self.year+self.month+self.day)
            
        if (self.gpm_type == "imerg/gis" or self.gpm_type=="imerg/gis/early"):
            server ="{0}/{1}/{2}/*{3}*".format(self.url,self.gpm_type,self.year+"/"+self.month,self.year+self.month+self.day)
        
        cmd = "{0} --user={1} --password={1} -qO- {2}".format(wget_path,self.userInfo,server)
    ...
    def get_file(self,filenames):
        ...
        cmd = "{0} -r -nd -P {1} --limit-rate=20000k --user={2} --password={3} --content-on-error {4}".format(wget_path,self.donwload_folder+"/"+ftype,self.userInfo,self.userpass,self.url+filenames)
