...
class cm_method:
    def __init__(self,input_raster,dist,input_csv,output_path,tbl_Result_CM):
        ...
        print (extent_layer_1[0],extent_layer_1[2],extent_layer_1[1].replace(",",""),extent_layer_1[3].split(">")[0])
        extent_layer="{0},{1},{2},{3}".format(extent_layer_1[0],extent_layer_1[2],extent_layer_1[1].replace(",",""),extent_layer_1[3].split(">")[0])
        ...
        if (os.path.splitext(input_csv)[1] == ".csv"):
            ...
            ra2 = self.SAT_idw(dist,(extent_layer),self.point2raster(input_csv,input_raster,output_path),colums,rows,pixelSizeX,output_path)
            ground=self.ground_idw(dist,(extent_layer),input_csv,colums,rows,pixelSizeX,output_path)
...
#             self.calc_CMmethod(input_raster,Diff,output_path)
    ...
    def ground_idw(self,distance_coefficient,extent,interporation_data,column,row,pixelsize,output_path):
        print ("GROUND_IDW")
        print (interporation_data)
#         output = output_path+"/"+os.path.basename(interporation_data).split(".")[0]+"_ground_idw.asc"
        output = output_path+"/"+(os.path.basename(os.path.splitext(interporation_data)[0]))+"_idw.tif"
...
#         output="D:/Working/KICT/2020/sample1/20200619/"+os.path.basename(interporation_data).split(".")[0]+"_idw.asc"
        if (os.path.basename(interporation_data).split(".")[0] == "ground_point_sample"):
            ...
            alg={'DISTANCE_COEFFICIENT' : distance_coefficient,
                 'EXTENT' : extent,
                 'INTERPOLATION_DATA' : interporation_data+'::~::0::~::1::~::0', # 이 부분 변경해야 함.
                 'OUTPUT' : output,
                 'COLUMNS':column,
                 'ROWS':row,
            ...
            alg={'DISTANCE_COEFFICIENT' : distance_coefficient,
                 'EXTENT' : extent,
                 'INTERPOLATION_DATA' : interporation_data+'::~::0::~::2::~::0', # 이 부분 변경해야 함.
                 'OUTPUT' : output,
                 'COLUMNS':column,
                 'ROWS':row,
                 'PIXEL_SIZE' :pixelsize
    ...
    def point2raster(self,input_shp,input_sat,output_path):
        print ("POINT2RASTER")
        output = output_path+"/"+(os.path.basename(os.path.splitext(input_sat)[0]))+"_RS.shp"
        print (output)
        alg={ 'COLUMN_PREFIX' : 'rvalue', 
            'INPUT' :input_shp, 
            'OUTPUT' : output, 
    ...
    def SAT_idw(self,distance_coefficient,extent,interporation_data,column,row,pixelsize,output_path):
        print ("SAT_IDW")
        print (interporation_data)
#         output = output_path+"/"+os.path.basename(interporation_data).split(".")[0]+"_idw.asc"
        output = output_path+"/"+os.path.basename(interporation_data).split(".")[0]+"_idw.tif"
...
#         output=output_path+"/"+os.path.basename(SAT2).split('.')[0]+"_"+os.path.basename(ground_idw).split(".")[0]+"_DIFF.asc"
        output=output_path+"/"+os.path.basename(SAT2).split('.')[0]+"_"+os.path.basename(ground_idw).split(".")[0]+"_DIFF.tif"
...
#         output="D:/Working/KICT/2020/sample1/20200619/"+os.path.basename(SAT2).split('.')[0]+"_"+os.path.basename(ground_idw).split(".")[0]+"_DIFF.asc"
        ...
        alg= 'gdal_calc --calc "A-B" --format GTiff --type Float32 -A "{0}" --A_band 1 -B "{1}" --B_band 1 --outfile "{2}" '.format(SAT2,ground_idw,output)
        print (alg)
#         os.system(alg)
    ...
    def calc_CMmethod(self,SAT,DIFF,output_path):
        output=output_path+"/"+(os.path.basename(os.path.splitext(SAT)[0]))+"_cm.tif"
        alg= 'gdal_calc --calc "A-B" --format GTiff --type Float32 -A "{0}" --A_band 1 -B "{1}" --B_band 1 --outfile "{2}" --NoDataValue={3}  '.format(SAT,DIFF,output,self.Nodata)
        subprocess.call(alg,shell=True)
        self.convert2asc(output)
...
#         crs='EPSG:4326'
        crs=self.Projection
        input_csv=csv
        Output_Layer=os.path.dirname(csv)+"/"+os.path.basename(csv).split(".csv")[0]+".shp"
        #field_1 field_2 = 좌표정보 인 csv
        uri="file:///{0}?type=csv&useHeader=No&detectTypes=yes&xField=field_1&yField=field_2&crs={1}&spatialIndex=no&subsetIndex=no&watchFile=no".format(input_csv,crs)
        print (uri)
        crs_sys = QgsCoordinateReferenceSystem(crs)
        layer_csv = QgsVectorLayer(uri,os.path.basename(input_csv) , 'delimitedtext')
        QgsVectorFileWriter.writeAsVectorFormat(layer_csv, Output_Layer,'utf-8', driverName ='ESRI Shapefile',destCRS=crs_sys)
        return Output_Layer
    ...
    def read_Layer_Get_info(self,ratserLayer):
        ...
        outputLayer = QgsRasterLayer(ratserLayer, ((os.path.basename(ratserLayer))), "gdal")
    ...
    def convert2asc(self,input_file):
        output = os.path.dirname(input_file)+"/"+(os.path.basename(input_file).split(".tif")[0])+".asc"
