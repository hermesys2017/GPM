...
class cm_method_run:
    def __init__(self,input_raster,dist,input_csv):
        ...
        ra2 = self.SAT_idw(dist,(ra.extent()),self.point2raster(input_csv,input_raster),colums,rows)
    ...
    def ground_idw(self,distance_coefficient,extent,interporation_data,column,row):
        output = os.path.dirname(interporation_data)+"/"+os.path.basename(interporation_data).split(".")[0]+"_idw.asc"
    ...
    def point2raster(self,input_shp,input_sat):
        print ("POINT2RASTER")
        output = os.path.dirname(input_sat)+"/"+os.path.basename(input_sat).split(".")[0]+"_RS.shp"
        print (output)
    ...
    def calculator_diff(self,SAT2,ground_idw):
        ...
        output=os.path.dirname(SAT2)+"/"+os.path.basename(SAT2).split('.')[0]+"_"+os.path.basename(ground_idw).split(".")[0]+"_DIFF.asc"
    ...
    def calc_CMmethod(self,SAT,DIFF):
        ...
        alg= 'gdal_calc --calc "A-B" --format GTiff --type Float32 -A "{0}" --A_band 1 -B "{1}" --B_band 1 --outfile "{2}" '.format(SAT,DIFF,output)
        os.system(alg)
        return output
