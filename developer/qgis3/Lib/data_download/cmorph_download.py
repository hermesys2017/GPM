...
#     /precip/CMORPH_V1.0/CRT/0.25deg-3HLY/2018/201811/CMORPH_V1.0_ADJ_0.25deg-3HLY_20181101.bz2
    ...
    for i in range(days.days+1):
        url="https://ftp.cpc.ncep.noaa.gov/precip/CMORPH_V1.0/CRT/0.25deg-3HLY/{0}/{1}/".format(str(start.strftime('%Y')), str(start.strftime('%Y%m')))
        #CMORPH_V1.0_ADJ_0.25deg-3HLY_20181101.bz2
        filename ="CMORPH_V1.0_ADJ_0.25deg-3HLY_{0}.bz2".format(str(start.strftime('%Y%m%d')))
        
        path = url+filename
        arg=wget_path+" -r -nd -P \"""{0}\" ".format(output+"/CMORPH/")+ "--limit-rate=20000k --content-on-error {0}".format(path)
